...
class SymplecticObject:
    def __init__(self, pts, yms, prs, rhos, appx_vol, num_handles=10, num_samples=1000, model_layers=6,
        ...
        r""" Initializing and training a symplectic object
        ...
        Args:
            ...
            model_layers (int, optional): Training parameter. Layers in the object network. Defaults to 6. 
        ...
        if warp_training:
            ...
            self.compute_losses = partial(compute_losses_H,
                                          batch_size=training_batch_size, 
                                          num_handles=self.num_handles,
                                          appx_vol=norm_appx_vol,
                                          num_samples=self.num_samples)
    ...
    def save_model(self, pth):
        r"""Saves the object network model (not including rigid mode)

        Args:
            pth (str): Path to object model file
        """
        torch.save(self.model, pth)

    def load_model(self, pth):
        r"""Loads the object network model from file. Adds rigid mode.
...
class SimulatedObject:
    def __init__(self, obj: SymplecticObject, num_cub_pts=1000, init_tfm=None):
        r""" Initialize a simulated object. Users should avoid direct access to this class. 
            ...
            Args:
                obj (SymplecticObject): Saves the symplectic Object to be used in the sim by reference here
                num_cub_pts (int, optional): Number of cubature points (integration primitives). Defaults to 1000
        """
        self.symplectic_object = obj
        ...
        """
        self.integration_sampling = (self.symplectic_object.appx_vol /
    ...
    def __str__(self):
        ...
        appx_vol = self.symplectic_object.appx_vol.item()
    ...
    def _sample_cubatures(self, num_cub_pts):
        r"""Internal function to sample cubature points over the object. 
        ...
        """
        self.sample_indices = torch.randint(low=0, high=self.symplectic_object.normalized_pts.shape[0], size=(
            num_cub_pts,), device=self.symplectic_object.default_device)
        self.sim_pts = self.symplectic_object.pts[self.sample_indices]
        self.sim_normalized_pts = self.symplectic_object.normalized_pts[self.sample_indices]
        self.sim_yms = self.symplectic_object.yms[self.sample_indices]
        self.sim_prs = self.symplectic_object.prs[self.sample_indices]
        self.sim_rhos = self.symplectic_object.rhos[self.sample_indices]
    ...
    def _set_sim_constants(self):
        ...
        self.weights = self.symplectic_object.model_plus_rigid(self.sim_normalized_pts)
        self.M, self.invM = precomputed.lumped_mass_matrix(self.sim_rhos, self.symplectic_object.appx_vol, dim=3)
        self.bigI = torch.tile(torch.eye(3, device=self.symplectic_object.default_device).flatten(
        ).unsqueeze(dim=1), (self.num_cub_pts, 1)).detach()

        self.dFdz = precomputed.jacobian_dF_dz(self.symplectic_object.model_plus_rigid,
    ...
    def reset_sim_state(self, init_tfm=None):
        self.z = torch.zeros((self.symplectic_object.num_handles + 1) * 12,
                             dtype=self.symplectic_object.default_dtype, device=self.symplectic_object.default_device).unsqueeze(-1)
        if init_tfm is not None:
            self.z[-12:, 0] = init_tfm.flatten()
        self.z_prev = self.z.clone().detach()
        self.z_dot = torch.zeros_like(self.z, device=self.symplectic_object.default_device)

        #p = torch.zeros_like(z, device=device)
        self.p = torch.zeros_like(self.z, device=self.symplectic_object.default_device)

        # compute_grad_potential_fn = partial(SimplicitsSceneTest._compute_grad_potential, B=self.B, x0_flat=self.x0_flat, dFdz=self.dFdz, bigI=self.bigI, defo_grad_energies=self.get_all_force_fcns("defo_grad_wise", "energy"), pt_wise_energies=self.get_all_force_fcns("pt_wise", "energy"))  
        # grad_V = compute_grad_potential_fn(self.z)
        # self.p = -self.timestep * grad_V
...
class SymplecticScene:
    ...
    def __init__(self,
                 ...
                 max_newton_steps=20,
                 max_ls_steps=30):
        r"""Initializes a symplectic scene. This is the entry point to easy API. 
        SymplecticObjects can be added to the scene. 
    ...
    def add_object(self, sim_object: SymplecticObject, num_cub_pts=1000, init_tfm=None):
        r"""Adds a object to the scene as a SimulatedObject.

        Args:
            sim_object (SymplecticObject): Symplectic object that will be wrapped into a SimulatedObject for this scene.
    ...
    def get_object(self, obj_idx):
        ...
        Returns:
            SimulatedObject: Simulated Object used by the scene. Also contains ref to object.
        """
    ...
    def get_object_deformed_pts(self, obj_idx, points=None):
        ...
        if points == None:
            points = obj.symplectic_object.pts
        return simplicits_utils.weight_function_lbs(points, tfms=obj.z.reshape(-1, 3, 4).unsqueeze(0), fcn=obj.symplectic_object.model_plus_rigid)
    ...
    def get_object_deformation_gradient(self, obj_idx, points=None):
        ...
        fcn_get_x = partial(simplicits_utils.weight_function_lbs, tfms=obj.z.reshape(-1,
                            3, 4).unsqueeze(0), fcn=obj.symplectic_object.model_plus_rigid)
    ...
    def _compute_grad_potential(self, z, B, x0_flat, dFdz, bigI, defo_grad_energies=[], pt_wise_energies=[]):
        """
        Compute ∇V(z), total potential gradient wrt z.
        ...
        pe_sum = torch.tensor([0], device=self.default_device, dtype=self.default_dtype)
        self._potential_sum(pe_sum, z=z, z_dot=None, B=B, dFdz=dFdz, x0_flat=x0_flat, bigI=bigI, defo_grad_fcns=defo_grad_energies, pt_wise_fcns=pt_wise_energies)
    ...
    # p need to initialize outside simulation step
    ...
    def run_sim_step(self):
        ...
        for idx in self.sim_obj_dict:
            ...
            compute_grad_potential_fn = partial(self._compute_grad_potential, B=o.B, x0_flat=o.x0_flat, dFdz=o.dFdz, bigI=o.bigI, defo_grad_energies=o.get_all_force_fcns("defo_grad_wise", "energy"), pt_wise_energies=o.get_all_force_fcns("pt_wise", "energy"))

            grad_V = compute_grad_potential_fn(o.z)
            
            o.p = o.p - self.timestep * grad_V

            o.p = 0.90 * o.p  # apply damping
            o.z = o.z + self.timestep * torch.linalg.solve(o.BinvMB, o.p)
